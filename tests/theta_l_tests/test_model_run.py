...
def test_theta_baro_wave():
  ...
  model_config = init_config()
  test_config = get_umjs_config(model_config=model_config)
  model_state, _ = get_umjs_state(h_grid, v_grid, model_config, test_config, dims, mountain=False, hydrostatic=False, pert_type="exponential")
  total_time = (3600.0 * 24.0 * 30)/2.0
  end_state = simulate_theta(total_time, nx, model_state,
                                       h_grid, v_grid,
                                       model_config, dims,
                                       hydrostatic=True,
