...
from pysces.mesh_generation.element_local_metric import create_quasi_uniform_grid_unstructured, gen_metric_terms_unstructured
...
def test_gen_metric():
  for npt in test_npts:
    for nx in [6, 7]:
      grid, dims = create_quasi_uniform_grid_unstructured(nx, npt, wrapped=False)
