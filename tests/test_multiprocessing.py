...
from pysces.config import jnp, device_unwrapper, np, do_mpi_communication, npt, mpi_rank, mpi_size, mpi_comm, use_wrapper, device_wrapper
...
def test_unordered_assembly_stub():
  for nx in range(1, 3):
    for nproc in range(1, 8):
      ...
      grid_total, dim_total = create_quasi_uniform_grid(nx, jax=False)
...
def test_extract_fields_triples():
  np.random.seed(0)
  for nx in range(1, 3):
    global_grids_nojax = [create_uniform_grid(nx, nx+1, jax=False), create_quasi_uniform_grid(nx, jax=False)]
    global_grids = [create_uniform_grid(nx, nx+1, jax=use_wrapper), create_quasi_uniform_grid(nx, jax=use_wrapper)]
