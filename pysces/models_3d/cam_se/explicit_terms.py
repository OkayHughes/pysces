...
from .thermodynamics import sum_species, midpoint_pressure, interface_pressure, cp_moist, hydrostatic_geopotential
...
def init_common_variables(dynamics, static_forcing, moisture_species, dry_air_species, h_grid, v_grid, physics_config, model):
  ...
  virtual_temperature = virtual_temperature_fn(total_mixing_ratio, temperature)
...
def temperature_horiz_advection_term(common_variables, h_grid, physics_config):
  grad_temperature = horizontal_gradient_3d(common_variables["T"], h_grid, physics_config)
...
def temperature_vertical_advection_term(common_variables, h_grid, physics_config):
  ...
  d_pressure_u = common_variables["d_pressure"] * u
...
def d_mass_advection_term(common_variables, h_grid, physics_config):
...
def energy_gradient_term(common_variables, h_grid, physics_config):
  ...
  phi = hydrostatic_geopotential(common_variables["pressure_midpoint"],
                                 common_variables["virtual_temperature"],
...
def pressure_gradient_force_term(common_variables, h_grid, v_grid, physics_config, pgf_formulation):
  ...
  kappa = common_variables["R_dry"] / cp_dry
  exner = (common_variables["pressure_midpoint"] / physics_config["p0"])**kappa
  ...
  T1 = (lapse_rate * T_ref * physics_config["reference_profiles"]["cp"] / physics_config["gravity"])
  T0 = T_ref - T1
  pgf_term_grad_exner = cp_dry[:, :, :, :, np.newaxis] * theta_v[:, :, :, :, np.newaxis] * grad_exner
  grad_logexner = horizontal_gradient_3d(np.log(exner), h_grid, physics_config)
  ...
  if pgf_formulation == pressure_gradient_options.grad_exner:
      ...
      pgf_term = -jnp.where(v_grid["hyb_m"][np.newaxis, np.newaxis, np.newaxis, :, np.newaxis] > 1e-9,
                            lower_levels_pgf,
                            basic_pgf)
  else:
...
def explicit_tendency(dynamics, static_forcing, moist_species, dry_species, physics_config, v_grid, h_grid, model, pgf_formulation=pressure_gradient_options.corrected_grad_exner):

  common_variables = init_common_variables(dynamics, static_forcing, moist_species, dry_species, h_grid, v_grid, physics_config, model)
  ...
  d_mass_tend = d_mass_advection_term(common_variables, h_grid, physics_config)
