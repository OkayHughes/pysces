...
from .hyperviscosity import init_hypervis_config_stub, init_hypervis_config_tensor, hypervis_opts
...
def init_default_config(nx, h_grid, v_grid, dims, model, physics_dt=-1.0, hypervis_type=hypervis_opts.variable_resolution):
  if physics_dt < 0:
  ...
  if hypervis_type is hypervis_opts.variable_resolution:
    ...
    diffusion_config = init_hypervis_config_tensor(nx, physics_config, v_grid)
  elif hypervis_type is hypervis_opts.none:
    diffusion_config = init_hypervis_config_stub()
  timestep_config = init_timestep_config(physics_dt, h_grid, physics_config, diffusion_config, dims, model)
