...
from ..model_info import f_plane_models, deep_atmosphere_models, thermodynamic_variable_names, hydrostatic_models, cam_se_models, homme_models, variable_kappa_models, dry_mixing_ratio_models, moist_mixing_ratio_models
...
@partial(jit, static_argnames=["model"])
def wrap_tracer_struct(moisture_species, tracers, model, dry_air_species=None):
...
def remap_dynamics(dynamics_in, static_forcing, v_grid, physics_config, num_lev, model):
  ...
  if model not in hydrostatic_models:
    p_mid = get_p_mid(dynamics_in, v_grid)
